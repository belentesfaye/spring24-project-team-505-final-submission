...
export type TownEvents = {
  ...
  /**
   * An event that indicates that coins have been changed
   */
  coinsChanged: () => void;
  /**
   * An event that indicates that pets have been changed
   */
  petsChanged: () => void;
  /**
...
export default class TownController extends (EventEmitter as new () => TypedEmitter<TownEvents>) {
  ...
  registerSocketListeners() {
    ...
    /**
     * On coin changes, forward the messages to listeners who subscribe to the controller's events
     */
    this._socket.on('coinsChanged', () => {
      this.emit('coinsChanged');
    });

    /**
     * On pet changes, forward the messages to listeners who subscribe to the controller's events
     */
    this._socket.on('petsChanged', () => {
      this.emit('petsChanged');
    });
  ...
  /**
   * Emit a movement event for the current player, updating the state locally
   *
   * Note: it is the responsibility of the townService to set the 'interactableID' parameter
   * of the player's location, and any interactableID set here may be overwritten by the townService
   *
  ...
  /**
   * Create a new petshop area, sending the request to the townService. Throws an error if the request
   * is not successful. Does not immediately update local state about the new conversation area - it will be
   * updated once the townService creates the area and emits an interactableUpdate
   *
   * @param newArea
   */
  // async createPetShopArea(newArea: { id: string; occupants: Array<string> }) {
  //   await this._townsService.createPetShopArea(this.townID, this.sessionToken, newArea);
  // }
