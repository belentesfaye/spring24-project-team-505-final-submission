...
export default class Town {
   ...
   * pet shop area with the same ID. The pet shop area ID must match the name of a
   * pet shop area that exists in this town's map.
   *
   ...
   *      emitting an interactableUpdate event
   *
   * @param petShopArea Information describing the pet shop area to create.
   *
   * @returns True if the pet shop area was created or false if there is no known
   * pet shop area with the specified ID or if there is already an active pet shop area
   * with the specified ID
   */
  ...
  public initializeFromMap(map: ITiledMap) {
    ...
    // const petShopAreas = objectLayer.objects
    //   .filter(eachObject => eachObject.type === 'petShopArea')
    //   .map(eachPetShopAreaObject =>
    //     PetShopArea.fromMapObject(eachPetShopAreaObject, this._broadcastEmitter),
    //   );
